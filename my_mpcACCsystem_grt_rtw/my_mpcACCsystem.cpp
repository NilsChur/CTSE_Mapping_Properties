...
{ 
...

  {
     ...
     */
    my_mpcACCsystem_B.e_v = my_mpcACCsystem_P.v_set - my_mpcACCsystem_B.Sum1; // !!! Calc error velocity
    ...
    my_mpcACCsystem_B.e_d = my_mpcACCsystem_B.safe_distance -
      my_mpcACCsystem_B.d_rel; // !!! Calc error distance


    /*Time for the Observer*/
    time_T timing = rtsiGetT(&(&my_mpcACCsystem_M)->solverInfo);

    /* APs definitions */
    /* Velocity */
    // APV[safeV] = (my_mpcACCsystem_B.e_v < 0.05*my_mpcACCsystem_b.v_set);
    // APV[riseV] = (my_mpcACCsystem_B.e_v <= 0.1 && timing < 10.3);

    // /* Distinguish between overshoot and the "undershoot" (overshoot) if the car is faster than the reference */
    // if (my_mpcACCsystem_B.Sum1 >= my_mpcACCsystem_b.v_set)
    // {
    //   APV[overshootV] = (my_mpcACCsystem_B.Sum1 <= 0.9 * my_mpcACCsystem_b.v_set);
    // }
    // else
    // {
    //   APV[overshootV] = (my_mpcACCsystem_B.Sum1 > 1.1 * my_mpcACCsystem_B.v_set);
    // }
    // APV[settlingV] = (my_mpcACCsystem_B.e_v < 0.05 && timing < 45);
    ...
    AP[rise] = ((my_mpcACCsystem_B.d_rel - my_mpcACCsystem_B.safe_distance) >= -0.10 * my_mpcACCsystem_B.safe_distance && timing <= 14.9 && timing >= 10.0); // Case1: 8.6; Case2: 4.1; Case3: 16.5

    AP[settling] = ((my_mpcACCsystem_B.d_rel - my_mpcACCsystem_B.safe_distance) > 0.05 * my_mpcACCsystem_B.safe_distance && timing <= 4.6);// && timing >= 3.8); // Case1: 9.74; Case2: 4.6; Case3: error
    
    AP[overshoot] = (my_mpcACCsystem_B.d_rel < my_mpcACCsystem_P.D_default);
...
my_mpcACCsystemModelClass::my_mpcACCsystemModelClass() :
  ...
  my_mpcACCsystem_X(),
  my_mpcACCsystem_M(),
  // APsV{{{riseV, false}, {overshootV, false}, {safeV, false}, {settlingV, false}}},
