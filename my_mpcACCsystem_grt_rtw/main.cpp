...
int_T main(int_T argc, const char *argv[])
{
     ...
     * Four different states are needed for each büchi automaton, therefore, eight states in total
    */

    /* Velocity Observer */
    int stateRiseVelocity = -1, oldStateRiseVelocity = 0;
    int stateOvershootVelocity = -1, oldStateOvershootVelocity = 0;
    int stateBoundedVelocity = -1, oldStateBoundedVelocity = 0;
    int stateSettlingTimeVelocity = -1, oldStateSettlingTimeVelocity = 0;

    /* Distance Observer */
    int stateRiseDistance = -1, oldStateRiseDistance = 0;
    int stateOvershootDistance = -1, oldStateOvershootDistance = 0;
    int stateStableDistance = -1, oldStateStableDistance = 0;
    int stateSettlingTimeDistance = -1, oldStateSettlingTimeDistance = 0;
    ...
    while(true){    
        __dios_reschedule();
        oldStateRiseDistance = stateRiseDistance;
        oldStateOvershootDistance = stateOvershootDistance;
        oldStateSettlingTimeDistance = stateSettlingTimeDistance;
        oldStateStableDistance = stateStableDistance;
        ...
        /* Each Büchi automaton is updated for the Velocity Observer */
        // stateRiseVelocity = nextRise(stateRise, MODEL_INSTANCE.APV);
        // stateOvershootVelocity = nextOvershoot(stateOvershoot, MODEL_INSTANCE.APV);
        // stateBoundedVelocity = nextBounded(stateBounded, MODEL_INSTANCE.APV);
        // stateSettlingTimeVelocity = nextSettlingTime(stateSettlingTime, MODEL_INSTANCE.APV);

        // __dios_trace_f( "state rise velocity: %d -> %d", oldStateRiseVelocity, stateRiseVelocity );
        // __dios_trace_f( "state stable velocity: %d -> %d", oldStateSettlingTimeVelocity, stateSettlingTimeVelocity );
        // __dios_trace_f( "state overshoot velocity: %d -> %d", oldStateOvershootVelocity, stateOvershootVelocity );
        // __dios_trace_f( "state bounded velocity: %d -> %d", oldStateBoundedVelocity, stateBoundedVelocity );



        /* Each Büchi automaton is updated for the distance Observer */

        // stateRiseDistance = nextRise(stateRiseDistance, MODEL_INSTANCE.AP);
        // __dios_trace_f( "state rise distance: %d -> %d", oldStateRiseDistance, stateRiseDistance );

        stateOvershootDistance = nextOvershoot(stateOvershootDistance, MODEL_INSTANCE.AP);
        __dios_trace_f( "state overshoot distance: %d -> %d", oldStateOvershootDistance, stateOvershootDistance );


        // stateSettlingTimeDistance = nextSettlingTime(stateSettlingTimeDistance, MODEL_INSTANCE.AP);
        // __dios_trace_f( "state stable distance: %d -> %d", oldStateSettlingTimeDistance, stateSettlingTimeDistance );

        
        // stateStableDistance = nextStable(stateStableDistance, MODEL_INSTANCE.AP);
        // __dios_trace_f("stability: %d -> %d", oldStateStableDistance, stateStableDistance);
