...
void Observer::setExternalInput(double Input1, double Input2, double Input3, double Input4) {
    ...
    sysOut = Input3;
    time = Input4;
    // printf("Inputs: Ref(%d), E(%d), Out(%d), T(%d)\n", (int)(Input1), (int)(Input2), (int)(Input3), (int)(Input4));
    // __dios_trace_f("SS: %f", (Input3 - Input1));
}
...
void Observer::initialThreshold(double thr1, double thr2, double thr3, double thr4, double thr5) {
    epsilon = thr1/100;
    overshootLevel = 10;
    riseLevel = -0.05*reference;
    riseTime = thr4;
    settlingTime = thr5;
    // printf("Threshold initialised: %d, %d, %d, %d, %d\n", (int)epsilon, (int)(overshootLevel), (int)riseLevel, (int)riseTime, (int)settlingTime);
}
